...
<img width="3024" height="4032" alt="IMG_7792" src="https://github.com/user-attachments/assets/6ee1061e-572a-4045-937c-f930e6742d3a" />
...
<img width="3024" height="4032" alt="IMG_7793" src="https://github.com/user-attachments/assets/330dc48a-80a8-4510-a154-2cbb7370d0f8" />
...
<img width="485" height="407" alt="image" src="https://github.com/user-attachments/assets/5b78533d-f0f0-4b1f-b530-780be10d70c8" />
...
- เพราะใช้ฟอนต์ขนาด $5 \times 7$ พิกเซล โดย 1 ไบต์ควบคุมพิกเซลในแนวตั้ง (คอลัมน์) 8 บิต เรียงต่อกัน 5 คอลัมน์ในแนวนอนเพื่อรวมกันเป็นรูปตัว 'H'
2. หากเราสลับสายไฟระหว่างขา **D0** และ **D1** จะเกิดผลอย่างไรกับสัญญาณ SPI และหน้าจอจะติดหรือไม่?
- สัญญาณนาฬิกา (SCLK) กับข้อมูล (MOSI) จะสลับขากัน ทำให้จอรับข้อมูลไม่ถูกต้อง และหน้าจอจะไม่ติด (หรือแสดงภาพขยะ)
3. เหตุใดการแก้ไขพิกัด $(x, y)$ บน `s_oled_buffer` จึงไม่ทำให้ภาพบนหน้าจอจริงเปลี่ยนทันที จนกว่าจะมีการเรียกคำสั่ง `oled_flush()`?
- เพราะ s_oled_buffer เป็นแค่หน่วยความจำแรม (RAM) ของ ESP32 การแก้ไขพิกัดเป็นเพียงการเตรียมภาพในแรม ต้องเรียก oled_flush() ก่อน ESP32 จึงจะยิงส่งข้อมูลทั้งหมดผ่าน SPI ไปยังชิปควบคุมของ OLED เพื่อแสดงผลบนหน้าจอจริง
